...
func (list *DVList) onScroll(offset uint32) {
	...
	atomic.StoreUint32(list.curStartPoint,
		uint32((int(offset)*list.data.Len())/sbar.OffsetResolution))
	list.mux.RUnlock()
	select {
	case list.actChan <- &listAct{
		act: actScrollBody,
...
func newDVListRender(list *DVList) *dvListRender {
	dlr := &dvListRender{
		list:      list,
		scrollbar: sbar.NewSBar(list.onScroll, false),
	}
	dlr.overallContainer = fyne.NewContainerWithoutLayout()
	dlr.header = NewListHeaderRow(list.data.Fields(),
		sbar.BarWidth, list.onTapHeader, dlr.onDragHeader, nil,
		list.headerLook)
	return dlr
}
...
func (dlr *dvListRender) loadData(layoutsize fyne.Size) {
	...
	defer func() {
		...
		newoffset := uint32((startPoint * sbar.OffsetResolution) / dlr.list.data.Len())
...
func (dlr *dvListRender) getBottomAllowanceSize(layoutsize fyne.Size) fyne.Size {
	...
	newwidth := layoutsize.Width - sbar.BarWidth
...
func (dlr *dvListRender) layout(layoutsize fyne.Size, mode int) {
	...
	if mode == listLayoutRedoALL {
		dlr.header = NewListHeaderRow(dlr.list.data.Fields(),
			sbar.BarWidth, dlr.list.onTapHeader, dlr.onDragHeader, nil,
			dlr.list.headerLook)
	}
