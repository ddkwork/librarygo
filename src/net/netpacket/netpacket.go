...
//  ║  5  │SetEvent(event interface{})            │set a event for any work            │                        │                                  ║
//  ║  6  │SetEventsCap(cap int)                  │Set Events Cap                      │                        │                                  ║
//  ║  7  │Events() <-chan interface{}            │pop events                          │work life               │                                  ║
...
type (
	Interface interface {
		...
		SetEvent(event interface{})       //接受任意类型的消息事件
		SetEventsCap(cap int)             //设置消息事件数量
		Events() <-chan interface{}       //信道先进先出队列存储事件信号
		HandleEvent()                     //todo 多态业务只要分态处理事件即可，即：这里处理如何退出主程序，或者永远不要退出,其余的接口签名通通 panic("implement me") 即可
		HttpClient() httpClient.Interface //http客户端接口（转发，cookie，表单，容错，各种请求方式等的封装），todo add udp，wss
...
func (o *object) SetEvent(event interface{})       { o.events <- event }
func (o *object) SetEventsCap(cap int)             { o.events = make(chan interface{}, cap) }
func (o *object) Events() <-chan interface{}       { return o.events }
func (o *object) HandleEvent()                     { panic("implement me") }
func (o *object) HttpClient() httpClient.Interface { return o.httpClient }
