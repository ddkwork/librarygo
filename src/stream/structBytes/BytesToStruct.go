...
func (o *object) ReadValue(v reflect.Value, depth int) (ok bool) {
	...
	case reflect.Slice:
		if v.IsNil() {
			return c.Error("切片必须在解码之前初始化为正确的长度")
		}
		fallthrough
	case reflect.Array:
		l := v.Len()
	...
	case reflect.Int:
		var i int32
		if !c.Error(binary.Read(o.Buffer, binary.BigEndian, &i)) {
			return
		}
		v.SetInt(int64(i))
	...
	case reflect.Bool:
		boolValue := uint8(0)
		if !c.Error(binary.Read(o.Buffer, binary.BigEndian, &boolValue)) {
			return
		}
		v.SetBool(boolValue != 0)

	default:
		return c.Error(binary.Read(o.Buffer, binary.BigEndian, v.Addr().Interface()))
	}
	return true
}
